...
Architecture:
  START
    │
  supervisor          (extracts product/market, detects advisory vs decision)
    │
    ├──────────────────────────────────────────── advisory? → advisory_agent → END
    │
    ├─ market_agent  ─┐
    ├─ financial_agent─┤  (PARALLEL)
    └─ knowledge_agent─┘
           │
       quality_router  (conditional — can loop back to any agent)
           │
       strategy_agent
           │
       quality_router_2 (can retry strategy or loop back to research)
           │
...
# ─── Advisory node (inline — no separate file needed) ───────────────────────

from agents.advisory_agent.agent import run_advisory
from datetime import datetime, timezone

async def advisory_node(state: dict) -> dict:
    """Handle open-ended advisory queries — no GO/NO_GO, just good advice."""
    ...
    return {
        ...
        "strategy_decision":  {
            "decision":       "ADVISORY",
            "adjusted_score": None,
            "summary":        "Advisory response — no GO/NO_GO decision made.",
            "rationale":      ["This was an advisory query, not a specific investment decision."],
            "next_steps":     ["Review the recommendations above and define a specific opportunity to evaluate."],
        },
...
def route_after_supervisor(state: dict) -> str:
    routing = state.get("routing_decision", "parallel_research")
    if routing == "advisory":
        return "advisory"
...
def build_graph() -> StateGraph:
    ...
    # Supervisor → advisory OR parallel research
    ...
    # Parallel: financial and knowledge also start after supervisor
    ...
    # Research → quality router (all three connect here)
