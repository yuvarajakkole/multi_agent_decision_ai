...
async def run(
    ...
    Returns (llm_qualitative_output, deterministic_calculations).

    The deterministic calculations are always computed by code — never by the LLM.
    The LLM only provides qualitative interpretation (risk factors, macro summary).
    """
    llm  = get_fast_llm()
    ...
    # ── Deterministic calculations (code, not LLM) ─────────────────────
    ...
    # Merge LLM qualitative + code quantitative
    merged = {**llm_output, **calcs}   # calcs override any LLM numbers
    return merged, calcs
