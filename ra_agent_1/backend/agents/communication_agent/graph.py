...
async def communication_agent_node(state: dict) -> dict:
    ...
    mc          = state.get("market_confidence",    0.0)
    fc          = state.get("financial_confidence", 0.0)
    kc          = state.get("knowledge_confidence", 0.0)
    ...
    print(f"[communication_agent] END  chars={len(report)}  decision={sd.get('decision')}")
    return {
        "final_report":       report,
